...
constexpr int ENEMY0_STRENGTH{ 9 };
constexpr int ENEMY1_STRENGTH{ 2 };
constexpr int ENEMY2_STRENGTH{ 10 };
constexpr int ENEMY3_STRENGTH{ 340 };
constexpr int ENEMY4_STRENGTH{ 2 };
constexpr int ENEMY5_STRENGTH{ 10 };
constexpr int ENEMY6_STRENGTH{ 170 };
constexpr int ENEMY7_STRENGTH{ 765 };
constexpr int ENEMY8_STRENGTH{ 3 };
constexpr int ENEMY9_STRENGTH{ 150 };
...
constexpr int ENEMY6_BASE_Y{ 100 };

constexpr int ENEMY7_BASE_X{ 1100 };
constexpr int ENEMY7_BASE_Y{ 460 };

constexpr int ENEMY8_BASE_X{ 1100 };
constexpr int ENEMY8_BASE_Y{ 332 };

constexpr int ENEMY9_BASE_X{ 1100 };
constexpr int ENEMY9_BASE_Y{ 212 };

using namespace std::string_literals;

const std::string Level8::m_state_ID{ "LEVEL7"s };
...
void Level8::update()
{
	...
	if (mouse_pos->get_x() > ENEMY0_BASE_X && mouse_pos->get_x() < ENEMY0_BASE_X + 130 && mouse_pos->get_y() > ENEMY0_BASE_Y && mouse_pos->get_y() < ENEMY0_BASE_Y + 130 && Input_handle::instance()->get_mouse_state(LEFT) == true) {
	...
	else if (is_tower_destroyed(0, 2) && mouse_pos->get_x() > ENEMY6_BASE_X && mouse_pos->get_x() < ENEMY6_BASE_X + 130 && mouse_pos->get_y() > ENEMY6_BASE_Y && mouse_pos->get_y() < ENEMY6_BASE_Y + 130 && Input_handle::instance()->get_mouse_state(LEFT) == true) {
	...
	else if (is_tower_destroyed(0, 6) && mouse_pos->get_x() > ENEMY7_BASE_X && mouse_pos->get_x() < ENEMY7_BASE_X + 130 && mouse_pos->get_y() > ENEMY7_BASE_Y && mouse_pos->get_y() < ENEMY7_BASE_Y + 130 && Input_handle::instance()->get_mouse_state(LEFT) == true) {
	...
	else if (is_tower_destroyed(0, 6) && mouse_pos->get_x() > ENEMY8_BASE_X && mouse_pos->get_x() < ENEMY8_BASE_X + 130 && mouse_pos->get_y() > ENEMY8_BASE_Y && mouse_pos->get_y() < ENEMY8_BASE_Y + 130 && Input_handle::instance()->get_mouse_state(LEFT) == true) {
		if (m_enemy[8] != nullptr) {
			...
			//if (m_player->get_strength() > m_enemy[8]->get_strength()) {
				...
				m_enemy[8]->set_strength_to_one();
				m_enemy[8]->set_current_frame(100);
			/*}
			else m_player->set_strength_to_zero();*/
		}
	}
	else if (is_tower_destroyed(0, 6) && mouse_pos->get_x() > ENEMY9_BASE_X && mouse_pos->get_x() < ENEMY9_BASE_X + 130 && mouse_pos->get_y() > ENEMY9_BASE_Y && mouse_pos->get_y() < ENEMY9_BASE_Y + 130 && Input_handle::instance()->get_mouse_state(LEFT) == true) {
...
void Level8::render()
{
	...
	if (!m_exit && m_enemy[1] != nullptr) {
		...
		if (m_enemy[1]->get_strength() == 1) Texture_manager::instance()->remove_from_texture_map("mul1"s);

	}
	else return;
	if (!m_exit && m_enemy[2] != nullptr) {
		m_enemy[2]->draw();
		m_enemy[2]->strength_to_text(ENEMY2_BASE_X + 60, ENEMY2_BASE_Y + 15);
		if (m_enemy[2]->get_strength() == 0) Texture_manager::instance()->remove_from_texture_map("minus1"s);
	}
	else return;
	...
	if (!m_exit && m_enemy[4] != nullptr) {
		...
		if (m_enemy[4]->get_strength() == 1) Texture_manager::instance()->remove_from_texture_map("div"s);
	}
	else return;
	...
	if (!m_exit && m_enemy[6] != nullptr) {
		m_enemy[6]->draw();
		m_enemy[6]->strength_to_text(ENEMY6_BASE_X + 40, ENEMY6_BASE_Y + 15);
	}
	else return;
	...
	if (!m_exit && m_enemy[8] != nullptr) {
		...
		if (m_enemy[8]->get_strength() == 1) Texture_manager::instance()->remove_from_texture_map("mul3"s);
	}
	else return;
	if (!m_exit && m_enemy[9] != nullptr) {
		m_enemy[9]->draw();
		m_enemy[9]->strength_to_text(ENEMY9_BASE_X + 60, ENEMY9_BASE_Y + 15);
		if (m_enemy[9]->get_strength() == 0) Texture_manager::instance()->remove_from_texture_map("minus2"s);
	}
	else return;
}

bool Level8::on_start()
{
	...
	if (!Texture_manager::instance()->create_texture_from_string("x"s, "mul3"s, 255, 248, 165, 255, 1, game::instance()->get_renderer())) return false;
	...
	m_object.push_back(new Game_object("base"s, ENEMY5_BASE_X, ENEMY5_BASE_Y, 130, 130));
	m_object.push_back(new Game_object("roof"s, ENEMY6_BASE_X, ENEMY6_BASE_Y, 130, 130));
	m_object.push_back(new Game_object("base"s, ENEMY7_BASE_X, ENEMY7_BASE_Y, 130, 130));
	m_object.push_back(new Game_object("base"s, ENEMY8_BASE_X, ENEMY8_BASE_Y, 130, 130));
	m_object.push_back(new Game_object("roof"s, ENEMY9_BASE_X, ENEMY9_BASE_Y, 130, 130));
...
bool Level8::on_exit()
{
	...
	Texture_manager::instance()->remove_from_texture_map("mul3"s);
	Texture_manager::instance()->remove_from_texture_map("minus1"s);
	Texture_manager::instance()->remove_from_texture_map("minus2"s);
...
bool Level8::is_tower_destroyed(int start_enemy_index, int end_enemy_index)
{
