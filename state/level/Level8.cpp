...
constexpr int ENEMY7_STRENGTH{ 1036 };
constexpr int ENEMY8_STRENGTH{ 3 };
constexpr int ENEMY9_STRENGTH{ 140 };
...
void Level8::update()
{
	...
	else if (is_tower_destroyed(0, 6) && mouse_pos->get_x() > ENEMY8_BASE_X && mouse_pos->get_x() < ENEMY8_BASE_X + BASE_WIDTH && mouse_pos->get_y() > ENEMY8_BASE_Y && mouse_pos->get_y() < ENEMY8_BASE_Y + BASE_HEIGHT && Input_handle::instance()->get_mouse_state(LEFT) == true && m_player->get_strength() > 0 && m_enemy[8]->get_strength() > 1) {
	...
	else if (is_tower_destroyed(0, 6)&&mouse_pos->get_x() > ENEMY9_BASE_X && mouse_pos->get_x() < ENEMY9_BASE_X + BASE_WIDTH && mouse_pos->get_y() > ENEMY9_BASE_Y && mouse_pos->get_y() < ENEMY9_BASE_Y + BASE_HEIGHT && Input_handle::instance()->get_mouse_state(LEFT) == true && m_enemy[9]->get_strength() > 0 && m_player->get_strength() > 0) {
	...
	if (m_player->get_strength() == 0 && frame_check_defeat()) {
		...
		game::instance()->set_level_cleared(8);
		SDL_Delay(500);
...
void Level8::render()
{
	...
	if (!m_exit && m_enemy[1] != nullptr) {
		m_enemy[1]->draw();
		m_enemy[1]->strength_to_text(ENEMY1_BASE_X + 50, ENEMY1_BASE_Y + 15,"x"s,true);
	...
	if (!m_exit && m_enemy[8] != nullptr) {
		m_enemy[8]->draw();
		m_enemy[8]->strength_to_text(ENEMY8_BASE_X + 50, ENEMY8_BASE_Y + 15,"x"s,true);
	}
	else return;
