...
void Level1::update()
{
	...
	if (mouse_pos->get_x() > ENEMY0_BASE_X && mouse_pos->get_x() < ENEMY0_BASE_X+ BASE_WIDTH && mouse_pos->get_y() > ENEMY0_BASE_Y&& mouse_pos->get_y() < ENEMY0_BASE_Y+ BASE_HEIGHT &&Input_handle::instance()->get_mouse_state(LEFT)==true) {
	...
	else if (mouse_pos->get_x() > ENEMY1_BASE_X && mouse_pos->get_x() < ENEMY1_BASE_X+ BASE_WIDTH && mouse_pos->get_y() > ENEMY1_BASE_Y&& mouse_pos->get_y() < ENEMY1_BASE_Y+ BASE_HEIGHT && Input_handle::instance()->get_mouse_state(LEFT) == true) {
...
void Level1::render()
{
	...
	if (!m_exit&&m_player != nullptr)
	{
		...
		/*else if (m_player->get_strength() != 0 && m_player->get_attack()){
			if (m_player->get_current_frame() < 3) {
	...
	if (!m_exit && m_enemy[0] != nullptr) {
		...
		if (m_enemy[0]->get_strength() == 0 && m_enemy[0]->get_current_frame() != 4) {
			int frame = m_enemy[0]->get_current_frame();
			frame_time = SDL_GetTicks() - start_time;
			start_time = SDL_GetTicks();
			if (frame_time > 100) {
	...
	if (!m_exit && m_enemy[1] != nullptr) {
		...
		if (m_enemy[1]->get_strength() == 0&&m_enemy[1]->get_current_frame()!=4) {
