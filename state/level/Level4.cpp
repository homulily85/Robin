...
constexpr int ENEMY1_BASE_X{ PLAYER_BASE_X + BASE_WIDTH * 5 };
constexpr int ENEMY1_BASE_Y{ PLAYER_BASE_Y };
constexpr int ENEMY2_BASE_X{ PLAYER_BASE_X + BASE_WIDTH * 5 };
constexpr int ENEMY2_BASE_Y{ PLAYER_BASE_Y-BASE_HEIGHT };
constexpr int ENEMY3_BASE_X{ PLAYER_BASE_X + BASE_WIDTH * 5 };
constexpr int ENEMY3_BASE_Y{ PLAYER_BASE_Y - BASE_HEIGHT*2 };
constexpr int ENEMY4_BASE_X= PLAYER_BASE_X + BASE_WIDTH * 6.5 ;
constexpr int ENEMY4_BASE_Y{ PLAYER_BASE_Y };
constexpr int ENEMY5_BASE_X = PLAYER_BASE_X + BASE_WIDTH * 6.5;
...
void Level4::update()
{
	...
	if (mouse_pos->get_x() > ENEMY0_BASE_X && mouse_pos->get_x() < ENEMY0_BASE_X + BASE_WIDTH && mouse_pos->get_y() > ENEMY0_BASE_Y && mouse_pos->get_y() < ENEMY0_BASE_Y + BASE_HEIGHT && Input_handle::instance()->get_mouse_state(LEFT) == true && m_enemy[0]->get_strength() > 0 && m_player->get_strength() > 0) {
	...
	else if (is_tower_destroyed(0, 0) && mouse_pos->get_x() > ENEMY1_BASE_X && mouse_pos->get_x() < ENEMY1_BASE_X + BASE_WIDTH && mouse_pos->get_y() > ENEMY1_BASE_Y && mouse_pos->get_y() < ENEMY1_BASE_Y + BASE_HEIGHT && Input_handle::instance()->get_mouse_state(LEFT) == true && m_player->get_strength() > 0 && m_enemy[1]->get_strength() > 1) {
		if (m_enemy[1] != nullptr) {
			...
			if (m_player->get_strength() > m_enemy[1]->get_strength()) {
				...
				m_enemy[1]->change_texture("witch_death"s, WITCH_WIDTH, WITCH_HEIGHT, SDL_FLIP_HORIZONTAL);
				m_player->change_texture("player_attack"s, PLAYER_WIDTH, PLAYER_HEIGHT, SDL_FLIP_NONE);
				m_player->set_attack(true);
			}
			else {
				m_player->set_strength_to_zero();
				m_player->change_texture("player_death"s, PLAYER_WIDTH, PLAYER_HEIGHT, SDL_FLIP_NONE);
				m_enemy[1]->change_texture("witch_attack"s, WITCH_WIDTH, WITCH_HEIGHT, SDL_FLIP_HORIZONTAL);
				m_enemy[1]->set_attack(true);
			}
		}
	}
	else if (is_tower_destroyed(0, 0) && mouse_pos->get_x() > ENEMY2_BASE_X && mouse_pos->get_x() < ENEMY2_BASE_X + BASE_WIDTH && mouse_pos->get_y() > ENEMY2_BASE_Y && mouse_pos->get_y() < ENEMY2_BASE_Y + BASE_HEIGHT && Input_handle::instance()->get_mouse_state(LEFT) == true && m_enemy[2]->get_strength() > 0 && m_player->get_strength() > 0) {
	...
	else if (is_tower_destroyed(0, 0) && mouse_pos->get_x() > ENEMY3_BASE_X && mouse_pos->get_x() < ENEMY3_BASE_X + BASE_WIDTH && mouse_pos->get_y() > ENEMY3_BASE_Y && mouse_pos->get_y() < ENEMY3_BASE_Y + BASE_HEIGHT && Input_handle::instance()->get_mouse_state(LEFT) == true && m_player->get_strength() > 0 && m_enemy[3]->get_strength() > 1) {
	...
	else if (is_tower_destroyed(0, 3) && mouse_pos->get_x() > ENEMY4_BASE_X && mouse_pos->get_x() < ENEMY4_BASE_X + BASE_WIDTH && mouse_pos->get_y() > ENEMY4_BASE_Y && mouse_pos->get_y() < ENEMY4_BASE_Y + BASE_HEIGHT && Input_handle::instance()->get_mouse_state(LEFT) == true && m_enemy[4]->get_strength() > 0 && m_player->get_strength() > 0) {
	...
	if (m_player->get_strength() == 0 && frame_check_defeat()) {
		...
		game::instance()->set_level_cleared(4);
		SDL_Delay(500);
...
void Level4::render()
{
	...
	if (!m_exit && m_enemy[1] != nullptr) {
		...
		if (m_enemy[1]->get_strength() != 1 && !m_enemy[1]->get_attack()) {
			m_enemy[1]->set_current_frame((int)((SDL_GetTicks() / 100) % (WITCH_DEFAULT_MAX_FRAME - 1)));
		}
		if (m_enemy[1]->get_strength() == 1 && m_enemy[1]->get_current_frame() < WITCH_DEATH_MAX_FRAME - 1 && !m_player->get_attack()) {
		...
		else if (m_enemy[1]->get_attack() && m_enemy[1]->get_current_frame() < WITCH_ATTACK_MAX_FRAME - 1) {
			int frame = m_enemy[1]->get_current_frame();
			frame_time = SDL_GetTicks() - start_time;
			start_time = SDL_GetTicks();
			if (frame_time > 100) {
	...
	if (!m_exit && m_enemy[3] != nullptr) {
		m_enemy[3]->draw();
		m_enemy[3]->strength_to_text(ENEMY3_BASE_X + 50, ENEMY3_BASE_Y + 15,"x"s,true);
	}
	else return;
...
bool Level4::on_start()
{
	...
	m_object.push_back(new Game_object("base"s, ENEMY2_BASE_X, ENEMY2_BASE_Y, BASE_WIDTH, BASE_HEIGHT));//enemy2's base, tower 2
	m_object.push_back(new Game_object("base"s, ENEMY3_BASE_X, ENEMY3_BASE_Y, BASE_WIDTH, BASE_HEIGHT));//enemy3's base, tower 2
	m_object.push_back(new Game_object("base"s, ENEMY4_BASE_X, ENEMY4_BASE_Y, BASE_WIDTH, BASE_HEIGHT));//enemy4's base, tower 3
	m_object.push_back(new Game_object("base"s, ENEMY5_BASE_X, ENEMY5_BASE_Y, BASE_WIDTH, BASE_HEIGHT));//enemy5's base, tower 3

	//m_object.push_back(new Game_object("div"s, ENEMY1_BASE_X + 50, ENEMY1_BASE_Y + 15, Texture_manager::instance()->get_text_width("div"s), Texture_manager::instance()->get_text_height("div"s)));
	...
	m_enemy.push_back(new Enemy("witch_idle"s, ENEMY1_STRENGTH, ENEMY1_BASE_X + 0.5 * (BASE_WIDTH - WITCH_WIDTH), ENEMY1_BASE_Y + WITCH_Y_SCALE, WITCH_WIDTH, WITCH_HEIGHT, SDL_FLIP_HORIZONTAL)); enemy_count++;//enemy1
	...
	m_enemy.push_back(new Enemy("buff"s, ENEMY3_STRENGTH, ENEMY3_BASE_X + 0.5 * (BASE_WIDTH - BUFF_WIDTH), ENEMY3_BASE_Y + BUFF_Y_SCALE, BUFF_WIDTH, BUFF_HEIGHT, SDL_FLIP_HORIZONTAL)); enemy_count++;//enemy3
...
bool Level4::frame_check_victory()
{
