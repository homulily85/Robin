...
constexpr int PLAYER_STRENGTH{ 16 };
constexpr int ENEMY0_STRENGTH{ 10 };
constexpr int ENEMY1_STRENGTH{ 25 };
constexpr int ENEMY2_STRENGTH{ 28 };
...
const std::string Level2::m_state_ID{ "LEVEL2"s };
...
void Level2::update()
{
	...
	else if (is_tower_destroyed(0, 0) && mouse_pos->get_x() > ENEMY1_BASE_X && mouse_pos->get_x() < ENEMY1_BASE_X + BASE_WIDTH && mouse_pos->get_y() > ENEMY1_BASE_Y && mouse_pos->get_y() < ENEMY1_BASE_Y + BASE_HEIGHT && Input_handle::instance()->get_mouse_state(LEFT) == true && m_enemy[1]->get_strength() > 0 && m_player->get_strength() > 0) {
...
bool Level2::is_tower_destroyed(int start_enemy_index, int end_enemy_index)
{
	for (int i = start_enemy_index; i <= end_enemy_index; i++) {
		if (!m_exit && m_enemy[i] != nullptr) {
			if (m_enemy[i]->get_strength() != 0) return false;
		}
	}
	return true;
}

bool Level2::frame_check_victory()
{
